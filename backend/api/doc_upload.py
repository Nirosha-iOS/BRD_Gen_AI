...
async def upload_document(
    ...
    """
    Upload document (PDF/DOC/Image), extract text, and store for BRD generation
    """
    try:
        # Save uploaded file
        file_extension = os.path.splitext(file.filename)[1]
        file_id = str(uuid.uuid4())
        doc_path = os.path.join("storage", "uploads", f"{file_id}{file_extension}")
        ...
        extracted_text = await process_document(doc_path, file_extension)
